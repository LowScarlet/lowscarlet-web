...
export default function CreativeCvContent({
  ...
  return (
    <div className="w-full max-w-[210mm] min-h-[297mm] print:min-h-0 print:w-full mx-auto grid grid-cols-12 rounded-none border border-slate-300 print:border-none print:shadow-none bg-white text-slate-900 shadow-xl font-sans text-left">
      ...
      <div className="col-span-8 bg-white p-4 sm:p-8 space-y-5 text-slate-900">
        {/* Main Header Inline */}
        <div className="border-b-2 border-slate-900 pb-4 mb-6 text-left">
          ...
          {profile.summary && (
            <p className="text-xs text-slate-700 leading-relaxed mt-3 pt-3 border-t border-slate-200 text-justify">
              {profile.summary}
            </p>
          )}
        </div>
            ...
            <div className="space-y-3">
              {volunteers.map((vol: any) => (
                <div key={vol.id} className="space-y-1">
                  <div className="flex flex-col sm:flex-row sm:items-baseline justify-between gap-1">
                    <h3 className="text-xs font-bold uppercase tracking-wider text-slate-900">
                      {vol.role} <span className="font-semibold text-slate-600">@ {vol.organization}</span>
                    </h3>
                    <span className="text-[10px] font-bold text-slate-500 shrink-0">
                      {vol.dateRange}
                    </span>
                  ...
                  {vol.highlights && vol.highlights.length > 0 && (
                    <ul className="list-disc list-inside text-xs text-slate-700 space-y-1 leading-relaxed pl-1 pt-0.5">
                      {vol.highlights.map((item: string, idx: number) => (
                        <li key={idx} className="marker:text-slate-900">
                          {item}
                        </li>
                      ))}
              ...
              {certifications.map((cert: any) => (
                <div key={cert.id} className="space-y-1">
                  ...
                  {cert.highlights && cert.highlights.length > 0 && (
                    <ul className="list-disc list-inside text-xs text-slate-700 space-y-0.5 leading-relaxed pl-1">
                      {cert.highlights.map((item: string, idx: number) => (
                        <li key={idx} className="marker:text-slate-900">
                          {item}
                        </li>
                      ))}
